...
# MODIFICA: La funzione ora restituisce anche il timestamp di caricamento
...
def load_and_prepare_data(url: str):
    ...
    try:
        ...
        # MODIFICA: Logica di fallback temperature
...
# MODIFICA: La funzione ora accetta il timestamp
def display_main_map(df, last_loaded_ts):
    ...
    # MODIFICA: Rimozione "Stamen Terrain"
    ...
    # MODIFICA: Usa il timestamp passato alla funzione
    if last_loaded_ts:
    ...
    # MODIFICA: Salvataggio stato mappa
    ...
    # Logica popup originale (funzionante)
    def create_popup_html(row):
        html = """<style>...</style><div class="popup-container">""" # Stile abbreviato
    ...
    # MODIFICA: Controllo robusto per TypeError
...
# MODIFICA: Funzione aggiornata con nuovi filtri e centraggio robusto
def display_period_analysis(df):
    ...
    for _, row in df_agg_filtered.iterrows():
        ...
        html_button = f"""<div style='text-align:center; margin-top:10px;'><a href='{link}' target='_self' style='...'>📈 Mostra Storico</a></div>"""
...
# MODIFICA: La pagina di dettaglio ora ha i grafici migliorati
...
# MODIFICA: Funzione main che gestisce il routing e passa il timestamp
def main():
    ...
    # Logica di routing originale (funzionante)
