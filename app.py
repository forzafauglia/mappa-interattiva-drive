...
if check_password():
    ...
    # Blocco robusto per caricare i file GeoJSON (Punti e Linee)
    for layer_info in layers_geojson:
        if layer_info['is_visible']:
            try:
                # Usa lo stile per le linee/poligoni
                style = {
                ...
                # Usa un colore di riempimento per i punti
                point_style = {
                    'fillColor': layer_info["style"]["color"],
                ...
                # Aggiunge l'intero GeoJSON alla mappa. Folium gestirà i diversi tipi di geometria.
                gjson = folium.GeoJson(
                    layer_info['url'],
                    name=layer_info['name'],
                    style_function=lambda x, style=style: style,
                    point_to_layer=lambda x, latlng: folium.CircleMarker(
                        location=latlng,
                        **point_style
                    ),
                    popup=folium.GeoJsonPopup(fields=['name', 'description'], aliases=['Nome:', 'Descrizione:']),
